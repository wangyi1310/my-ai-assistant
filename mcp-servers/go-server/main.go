...
// handleMakeAuthenticatedRequestTool is a tool that makes an authenticated request
// using the token from the context.
func handleSayHelloTool(
	ctx context.Context,
	request mcp.CallToolRequest,
) (*mcp.CallToolResult, error) {
	say, _ := request.Params.Arguments["say"].(string)
	return mcp.NewToolResultText(fmt.Sprintf("say hello %s", say)), nil
}
...
func NewMCPServer() *MCPServer {
	...
	mcpServer.AddTool(mcp.NewTool("make_say_hello_authenticated_request",
	...
	mcpServer.AddTool(mcp.NewTool("say_hello_everyone",
		mcp.WithDescription("Query detailed log information by specifying a unique request ID (reqid). Supports filtering logs from distributed systems."),
		mcp.WithString("say",
			mcp.Description("say to everyone"),
			mcp.Required(),
		),
	), handleSayHelloTool)
...
func main() {
	...
	case "sse":
		sseServer := s.ServeSSE("localhost:8081")
